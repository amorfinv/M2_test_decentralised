...
'''
Prepare the graph in Vienna for second round of hand edits.

Steps:
    1) Load 'hand_edits.gpkg' as an omsnx style gdf.
    2) Remove all self-loops.
    3) Run get_phantom_intersections to add nodes where edges intersect but don't have nodes
    4) Remove two way edges and long_way edges due to adding phantom intersections.
    5) Do a final clean of edge nameing. This removes any key=1 edge. TODO: consolidate with above functions.
    6) Run coins again for fun.
    7) Perform hand edits with QGIS in 'hand_edits.'
    8) Save graph and gdf as 'cleaning_process.*'
    9) Save a copy for hand edits as 'hand_edits_2.*'
'''
...
def main():
    ...
    edges = gpd.read_file(path.join(gis_data_path, 'streets', 'hand_edits.gpkg'), layer='edges')
    nodes = gpd.read_file(path.join(gis_data_path, 'streets', 'hand_edits.gpkg'), layer='nodes')
    

    # recreate edges in format for osmnx and remove self loops
    ...
    # save as osmnx graph
    ox.save_graphml(G, filepath=path.join(gis_data_path, 'streets', 'cleaning_process.graphml'))
    
    # # Save geopackage for import to QGIS and momepy
    ox.save_graph_geopackage(G, filepath=path.join(gis_data_path, 'streets', 'cleaning_process.gpkg'), directed=True)

    # create a graph that you need to edit
    override_flag = False ## !!!!!LEAVE AS FALSE OR WE LOSE HAND EDITS

    if override_flag:
        ox.save_graph_geopackage(G, filepath=path.join(gis_data_path, 'streets', 'hand_edits_2.gpkg'), directed=True)
